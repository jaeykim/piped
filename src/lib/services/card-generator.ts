...
function buildTextSvg(w: number, h: number, config: CardConfig, r: number, g: number, b: number): string {
  ...
  if (config.badge) {
    ...
    badgeSvgTop = `
      ...
      <text x="${pad + bPad}" y="${y + bH / 2 + bSize * 0.35}" font-family="'Helvetica Neue',Helvetica,Arial,sans-serif" font-weight="700" font-size="${bSize}" fill="#FFFFFF">${esc(config.badge)}</text>`;
    y += bH + ref * 0.015;
  }
  ...
  const headSvg = headLines.map((line, i) => {
    ...
    return `${highlightSvg}<text x="${pad}" y="${ly}" font-family="'Helvetica Neue',Helvetica,Arial,sans-serif" font-weight="900" font-size="${headSize}" fill="${textColor}" letter-spacing="-0.01em">${esc(line)}</text>`;
  }).join("");
  ...
  if (config.subheadline) {
    ...
    subSvg = subLines.slice(0, 3).map((line, i) => {
      return `<text x="${pad}" y="${y + subSize + i * (subSize * 1.4)}" font-family="'Helvetica Neue',Helvetica,Arial,sans-serif" font-weight="500" font-size="${subSize}" fill="${subColor}">${esc(line)}</text>`;
    }).join("");
  ...
  if (tags.length > 0) {
    ...
    for (const tag of tags.slice(0, 4)) {
      ...
      tagsSvg += `<rect x="${tx}" y="${y}" width="${tw}" height="${tagH}" rx="${tagH / 2}" fill="${tagBg}"/>
        <text x="${tx + tagPad}" y="${y + tagH / 2 + tagSize * 0.35}" font-family="'Helvetica Neue',Helvetica,Arial,sans-serif" font-weight="600" font-size="${tagSize}" fill="${tagText}">${esc(tag)}</text>`;
      tx += tw + tagGap;
    }
    y += tagH + ref * 0.015;
  }
  ...
  if (config.cta) {
    ...
    ctaSvg = `<rect x="${pad}" y="${y}" width="${ctaW}" height="${ctaH}" rx="${ctaH / 2}" fill="rgb(${r},${g},${b})"/>
      <text x="${pad + ctaW / 2}" y="${y + ctaH / 2 + ctaSize * 0.35}" font-family="'Helvetica Neue',Helvetica,Arial,sans-serif" font-weight="700" font-size="${ctaSize}" fill="#FFFFFF" text-anchor="middle">${esc(config.cta)}</text>`;
  }
  ...
  const brandSvg = `<rect x="${bx}" y="${by}" width="${bw}" height="${bh}" rx="${bh / 2}" fill="${isDark ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.05)"}"/>
    <text x="${bx + 8}" y="${by + bh / 2 + badgeSize * 0.35}" font-family="'Helvetica Neue',Helvetica,Arial,sans-serif" font-weight="600" font-size="${badgeSize}" fill="${isDark ? "rgba(255,255,255,0.5)" : "rgba(0,0,0,0.35)"}">${esc(config.productName)}</text>`;
